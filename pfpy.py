...
if __name__ == '__main__':
    ...
    assert (f ** d)(x) == (f ^ d)(x) == f(x) ** d
    assert (f << g)(x) == (f @ g)(x) == f(g(x))
    assert (f >> g)(x) == (g @ f)(x) == g(f(x))

    assert (abs @ g)(-1) == abs(g(-1))

    assert type(f) == type(g) == type(f + g)           # Additive closure
    assert type(f) == type(c * f)                      # Scalar closure
    assert (f + g)(x) == (g + f)(x)                    # Commutative
    assert (f + (g + h))(x) == ((f + g) + h)(x)        # Additive associativity
    assert (f + Z)(x) == f(x)                          # Additive identity (Zero vector)
    assert (f + (-f))(x) == Z(x)                       # Additive inverse
    assert (c * (d * f))(x) == ((c * d) * f)(x)        # Scalar associativity
    assert (c * (f + g))(x) == ((c * f) + (c * g))(x)  # Additive distributivity
    assert ((c + d) * f)(x) == ((c * f) + (d * f))(x)  # Scalar distributivity
    assert (1 * f)(x) == f(x)                          # Scalar identity (One)
